...
class ExailePlayer(object):
    ...
    def _on_message(self, bus, message, reading_tag=False):
        ...
        if handled:
            pass
        elif message.type == Gst.Message.TAG:
            ...
            if self.current and not self.current.get_tag_raw('__length'):
                res, raw_duration = self._pipe.query_duration(Gst.Format.TIME, None)
                if not res:
        ...
        elif message.type == Gst.Message.EOS and not self.is_paused():
            self._eos_func()
    ...
    def _setup_startstop_offsets(self, track):
        ...
        if start_offset > 0:
            ...
            if self._pipe.get_state(timeout=1000*Gst.MSECOND)[0] != Gst.StateChange.SUCCESS:
